        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Hierarchical Failure Containment: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study hierarchical robot policies. The submission-hardened claim is intentionally narrow: Design skill hierarchies that contain low-level failures before they corrupt task state. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Hierarchical failure containment should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.402 $\pm$ 0.013 & 0.445 $\pm$ 0.011 & 0.330 $\pm$ 0.007 \\
Data-augmented WAM & 0.489 $\pm$ 0.013 & 0.519 $\pm$ 0.023 & 0.433 $\pm$ 0.018 \\
Scalar uncertainty & 0.548 $\pm$ 0.017 & 0.575 $\pm$ 0.014 & 0.504 $\pm$ 0.016 \\
Ensemble disagreement & 0.592 $\pm$ 0.021 & 0.607 $\pm$ 0.017 & 0.547 $\pm$ 0.015 \\
Conformal risk filter & 0.610 $\pm$ 0.014 & 0.634 $\pm$ 0.022 & 0.574 $\pm$ 0.009 \\
Proposed branch mechanism & 0.681 $\pm$ 0.022 & 0.704 $\pm$ 0.013 & 0.659 $\pm$ 0.007 \\
Oracle branch upper bound & 0.762 $\pm$ 0.024 & 0.770 $\pm$ 0.011 & 0.750 $\pm$ 0.008 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.666 $\pm$ 0.007 & all components \\
Minus branch atlas & 0.557 $\pm$ 0.019 & removes explicit unrealized futures \\
Minus tail risk & 0.573 $\pm$ 0.009 & uses mean predicted value only \\
Minus diagnostic probe & 0.603 $\pm$ 0.016 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.601 $\pm$ 0.013 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/105.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Hierarchical Failure Containment is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
